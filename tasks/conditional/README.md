...
This is a source code for Conditional coverage task in the paper.
...
optional arguments:
  ...
  --num_groups NUM_GROUPS
                        Number of groups for groupwise coverage evaluation
  --file_path FILE_PATH 
                        File path for loading data
  --model MODEL         Model to use: ot_map, wasserstein, nonparametric

```

Example command:
```bash
python -m tasks.conditional.main \
